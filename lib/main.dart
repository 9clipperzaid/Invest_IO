...
void main() async {
  ...
  // ✅ FIX 1: Run dotenv + HiveService in PARALLEL (saves ~200-400ms)
  await Future.wait([
    dotenv.load(fileName: ".env"),
    HiveService.init(),
  ]);

  // ✅ FIX 2: Only open 2 critical boxes before launch (not all 14!)
  await HiveConfig.initCritical();

  // Non-async — no need to await these
  SystemChrome.setSystemUIOverlayStyle(
    const SystemUiOverlayStyle(
      statusBarColor: Colors.transparent,
      statusBarIconBrightness: Brightness.light,
  ...
  try {
    ...
    // ✅ FIX 3: Remaining 12 boxes open AFTER first frame — user sees UI instantly
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await HiveConfig.initRemaining();
    });
  } catch (error) {
    runApp(BootstrapErrorApp(message: _bootstrapErrorMessage(error)));
  }
...
String _bootstrapErrorMessage(Object error) {
  final text = error.toString();
  if (text.contains('Web Firebase is not configured')) {
    return 'Web Firebase is not configured yet.';
  }
  if (text.contains('Firebase is not configured for')) {
    return 'This project is configured for Android, iOS, and macOS only.';
  }
  return 'App bootstrap failed: $text';
}
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                const Text('Startup configuration issue',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                        color: Color(0xFFF1F5F9),
                        fontSize: 22,
