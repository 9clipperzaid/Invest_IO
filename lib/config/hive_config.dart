...
class HiveConfig {
  ...
  // ✅ NEW: Called at startup — opens ONLY what auth/splash needs
  // user_prefs: needed for dark mode, first-time flag, currency
  // holdings: needed if home screen shows portfolio immediately
  static Future<void> initCritical() async {
    await Hive.initFlutter(); // safe — HiveService already called this
    _registerAdapters();      // register all adapters (fast, in-memory only)
    await Future.wait([
      _safeOpen(userPrefsBox),
      _safeOpen(holdingsBox),
    ]);
  }

  // ✅ NEW: Called after first frame — loads remaining 12 boxes in background
  static Future<void> initRemaining() async {
    await Future.wait([
      _safeOpen(portfolioBox),
      _safeOpen(stockCacheBox),
      _safeOpen(newsCacheBox),
      _safeOpen(chatHistoryBox),
      _safeOpen(apiCacheBox),
      _safeOpen(apiCacheMetaBox),
      _safeOpen(bankAccountsBox),
      _safeOpen(transactionsBox),
      _safeOpenTyped<AttentionLogModel>(attentionLogsBox),
      _safeOpenTyped<BeliefLogModel>(beliefLogsBox),
      _safeOpenTyped<DecisionEventModel>(decisionEventsBox),
      _safeOpenTyped<BehaviorProfileModel>(behaviorProfileBox),
    ]);
  }

  // ── OLD full init (kept for reference — DO NOT USE in main.dart anymore) ──
  @Deprecated('Use initCritical() + initRemaining() instead')
  static Future<void> init() async {
    await Hive.initFlutter();
    _registerAdapters();
    await Future.wait([
  ...
  // ── Register adapters ──────────────────────────────────────────────────────
  static void _registerAdapters() {
    if (!Hive.isAdapterRegistered(20)) Hive.registerAdapter(AttentionLogModelAdapter());
    if (!Hive.isAdapterRegistered(21)) Hive.registerAdapter(BeliefLogModelAdapter());
    if (!Hive.isAdapterRegistered(22)) Hive.registerAdapter(DecisionEventModelAdapter());
    if (!Hive.isAdapterRegistered(23)) Hive.registerAdapter(BehaviorProfileModelAdapter());
  ...
  static Box<AttentionLogModel>    get attentionLogs   => Hive.box<AttentionLogModel>(attentionLogsBox);
  static Box<BeliefLogModel>       get beliefLogs      => Hive.box<BeliefLogModel>(beliefLogsBox);
  static Box<DecisionEventModel>   get decisionEvents  => Hive.box<DecisionEventModel>(decisionEventsBox);
  static Box<BehaviorProfileModel> get behaviorProfile => Hive.box<BehaviorProfileModel>(behaviorProfileBox);

  // ── User preferences helpers ───────────────────────────────────────────────
  static T? getPref<T>(String key) => Hive.box(userPrefsBox).get(key) as T?;
  static Future<void> setPref<T>(String key, T value) => Hive.box(userPrefsBox).put(key, value);

  static bool   get isDarkMode           => getPref<bool>('dark_mode') ?? true;
  static Future<void> setDarkMode(bool v)   => setPref('dark_mode', v);
  static bool   get isFirstTime          => getPref<bool>('is_first_time') ?? true;
  static Future<void> setFirstTime(bool v)  => setPref('is_first_time', v);
  static String get currency             => getPref<String>('currency') ?? 'INR';
  static Future<void> setCurrency(String v) => setPref('currency', v);
  static String get statedRiskTolerance  => getPref<String>('risk_tolerance') ?? 'medium';
  static Future<void> setRiskTolerance(String v) => setPref('risk_tolerance', v);
  static String get investingStyle       => getPref<String>('investing_style') ?? 'unknown';
  static Future<void> setInvestingStyle(String v) => setPref('investing_style', v);

  // ── Clear all data ─────────────────────────────────────────────────────────
  static Future<void> clearAll() async {
    await Future.wait([
      Hive.box(holdingsBox).clear(),
      Hive.box(portfolioBox).clear(),
      Hive.box(stockCacheBox).clear(),
      Hive.box(newsCacheBox).clear(),
      Hive.box(chatHistoryBox).clear(),
      Hive.box(userPrefsBox).clear(),
      Hive.box(apiCacheBox).clear(),
      Hive.box(apiCacheMetaBox).clear(),
      Hive.box<AttentionLogModel>(attentionLogsBox).clear(),
      Hive.box<BeliefLogModel>(beliefLogsBox).clear(),
      Hive.box<DecisionEventModel>(decisionEventsBox).clear(),
      Hive.box<BehaviorProfileModel>(behaviorProfileBox).clear(),
    ]);
  }
